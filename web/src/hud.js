...
export class HUD {
    ...
    tcEl = null;
    scte35Row = null;
    renderedSCTE35Ids = new Set();
    _externallyDriven = false;
    streamInfoEl = null;
    lastStreamInfoStr = "";
    lastBadgeLabels = new Map();
    lastBadgeStatuses = new Map();
    ...
    build() {
        ...
        const healthKeys = ["video", "audio", "sync"];
    ...
    render() {
        ...
        if (this.tcEl) {
            ...
            if (tc) {
                ...
                if (this.tcEl.textContent !== tc)
                    this.tcEl.textContent = tc;
            }
            else {
    ...
    renderSCTE35() {
        ...
        for (const event of visible) {
            ...
            const existing = this.scte35Row.querySelector(`[data-scte35-id="${id}"]`);
            if (existing) {
        ...
        for (const agoEl of this.scte35Row.querySelectorAll(".scte35-ago")) {
            const recv = Number(agoEl.dataset.receivedAt);
            if (!recv)
                continue;
            const secs = Math.floor((now - recv) / 1000);
            agoEl.textContent = secs < 1 ? "now" : `${secs}s ago`;
        }
    }
    buildSCTE35Toast(event, id) {
        const el = document.createElement("div");
        ...
        el.addEventListener("click", (e) => {
            e.stopPropagation();
            if (this.onSCTE35Click)
                this.onSCTE35Click(event);
        });
