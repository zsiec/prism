export function renderSparkline(canvas, data, color, fillAlpha = 0.15) {
...
export class DetailPanel {
    ...
    onClose = null;
    renderFn = null;
    constructor(container, _key, _store, zIndex) {
    ...
    addStatRow(section, id, label, initialValue = "—") {
...
export function buildVideoPanel(container, store, zIndex) {
...
export function buildAudioPanel(container, store, zIndex) {
    ...
    panel.setRenderFn(() => {
        ...
        if (a.tracks.length > 0) {
            ...
            if (a.tracks.length > 8)
                lines.push(`... +${a.tracks.length - 8} more`);
            panel.updateStat("a-detail", lines.join("\n"));
        }
    });
    return panel;
}
export function buildSyncPanel(container, store, zIndex) {
...
export function buildTransportPanel(container, store, zIndex) {
...
export function buildCaptionsPanel(container, store, zIndex) {
...
export function buildSCTE35Panel(container, store, zIndex, event) {
    const panel = new DetailPanel(container, "video", store, zIndex);
