...
class PrismAudioWorkletProcessor extends AudioWorkletProcessor {
    ...
    process(_inputs, outputs, _parameters) {
        if (!this.sharedStates || this.floatViews.length === 0)
            return true;
        ...
        if (available === 0) {
            this.outputSilence(outputs);
            Atomics.add(this.sharedStates, SharedStates.INSERTED_SILENCE_MS, Math.round((framesToFill / this.localSampleRate) * 1000));
    ...
    readFromRing(start, count, output, channels) {
    ...
    readChannelFromRing(start, count, channel, dst) {
    ...
    linearResample(src, srcLen, dst, dstLen) {
        if (srcLen === 0) {
            dst.fill(0);
            return;
        }
    ...
    computeLevels(output, channels, samples) {
        ...
        for (let c = 0; c < channels; c++) {
            ...
            Atomics.store(this.sharedStates, SharedStates.PEAK_BASE + c, Math.round(maxAbs * 1_000_000));
            Atomics.store(this.sharedStates, RMS_BASE + c, Math.round(rmsVal * 1_000_000));
        }
    }
}
