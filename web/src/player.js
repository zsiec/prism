...
import { HUD } from "./hud";
import { buildVideoPanel, buildAudioPanel, buildSyncPanel, buildTransportPanel, buildCaptionsPanel, buildSCTE35Panel, } from "./detail-panel";
...
export class PrismPlayer {
    ...
    renderer;
    vuMeter;
    audioTrackSelector = null;
    fullscreenBtn = null;
    perfOverlay;
    perfStartTime = 0;
    decodeFpsCounter = 0;
    lastDecodeFpsTime = performance.now();
    moqTransport = null;
    pendingVideoCodec = null;
    pendingVideoWidth = 0;
    pendingVideoHeight = 0;
    connectedStreamKey = null;
    destroyed = false;
    globalMute;
    reconnectDelay = 2000;
    constructor(container, options = {}) {
        ...
        this.renderer = new PrismRenderer(this.canvas, this.videoRenderBuffer, { getPlaybackPTS: () => this.primaryAudioDecoder?.getPlaybackPTS() ?? -1 }, (rendererStats) => {
    ...
    async connect(streamKey) {
        if (this.destroyed)
            return;
        return this.connectMoQ(streamKey);
    }
    async connectMoQ(streamKey) {
            ...
            this.moqTransport = new MoQTransport(streamKey, {
                onTrackInfo: async (tracks) => {
                    ...
                    if (videoTrack) {
                        if (videoTrack.initData) {
                            ...
                            this.videoDecoder.configure(videoTrack.codec, videoTrack.width, videoTrack.height, desc.buffer);
                        }
                        else {
                    ...
                    if (!this.globalMute) {
                        ...
                        if (audioTracks.length > 0 && !this.options.condensed) {
                            this.audioTrackSelector = new AudioTrackSelector(audioTracks, this.activeAudioTrack, (trackIndex) => this.switchAudioTrack(trackIndex), this.playerUI, () => this.enterLoudnessMode(), () => this.exitLoudnessMode());
                        }
                    }
                ...
                onVideoFrame: (data, isKeyframe, timestamp, _groupID, description) => {
                    ...
                    if (description && this.pendingVideoCodec) {
                        const descCopy = new Uint8Array(description).buffer;
                        this.videoDecoder.configure(this.pendingVideoCodec, this.pendingVideoWidth, this.pendingVideoHeight, descCopy);
     ...
     */
    async connectMux(streamKey, tracks, sharedAudioCtx, deferVideoConfig) {
        if (this.destroyed)
            return;
        this.disconnectInternal();
        this.stats.start();
    ...
    injectVideoFrame(data, isKeyframe, timestamp, description) {
        if (description && this.pendingVideoCodec) {
            const descCopy = new Uint8Array(description).buffer;
            this.videoDecoder.configure(this.pendingVideoCodec, this.pendingVideoWidth, this.pendingVideoHeight, descCopy);
            this.pendingVideoCodec = null;
    ...
    injectAudioFrame(data, timestamp, trackIndex) {
        const decoder = this.audioDecoders.get(trackIndex);
        if (decoder) {
            decoder.decode(data, timestamp, false);
        }
    ...
    setDecodersSuspended(suspended) {
        for (const [, decoder] of this.audioDecoders) {
    ...
    setupPanels() {
        ...
        this.hud.setOnSCTE35Click((event) => {
            this.closePanel();
            const panel = buildSCTE35Panel(this.playerUI.getContainer(), this.metricsStore, this.playerUI.getPanelZIndex(), event);
            panel.setOnClose(() => this.closePanel());
            panel.start();
            this.activeDetailPanel = panel;
    ...
    async setupAudioDecoders(audioTracks) {
        for (const [, decoder] of this.audioDecoders) {
            decoder.reset();
        }
        this.audioDecoders.clear();
